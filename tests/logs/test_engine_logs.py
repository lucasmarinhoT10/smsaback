...
from src.logs.handlers.file_handler import FileJSONHandler
...
class TestEngineLogs(unittest.TestCase):
    ...
    @patch.dict(os.environ, {"LOG_FILE": "true", "LOG_FILE_PATH": "test.log"})
    @patch("src.logs.engine_logs.FileJSONHandler")
    def test_adiciona_file_handler(self, mock_file_handler):
        engine_logs = EngineLogs()
        self.assertTrue(
            any(isinstance(h, mock_file_handler.return_value.__class__) for h in engine_logs.logger.handlers)
        )
        mock_file_handler.assert_called_with("test.log", maxBytes=5_000_000, backupCount=3)
